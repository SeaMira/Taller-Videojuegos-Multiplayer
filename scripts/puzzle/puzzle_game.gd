...
func check_pieces_on_cell(body, area:Area2D):
	if is_multiplayer_authority():
		...
		if bodies.size() == 2:
			...
			var b1_name = "celda_" + str(indxs.x) + "_" + str(indxs.y)
			
			var indxs2 = PuzzleSettings.search_orange_piece(bodies[1])
			if !(indxs2):
				var splitted2 = bodies[1].name.split("_")
				indxs2 = Vector2(float(splitted2[1]), float(splitted2[2]))
			var b2_name = "celda_" + str(indxs2.x) + "_" + str(indxs2.y)
